...
from utils.helpers import get_download_link, download_drive_file, format_branch_text, format_semester_text
...
def register_syllabus_handlers(bot: TeleBot, analytics: Analytics, user_session: UserSession):
    ...
    def semester_after_branch(message):
        try:
            ...
            file_bytes = download_drive_file(original_url)
            ...
            if file_bytes is not None:
                try:
                    bot.send_document(
                        message.chat.id,
                        file_bytes,
                        visible_file_name=f"{semester}_{branch}_Syllabus.pdf",
                        caption=caption,
                        reply_markup=markup,
                        parse_mode='Markdown'
                    )
            ...
            if file_bytes is None:
                bot.send_message(
                    message.chat.id,
                    f"{caption}\n\n"
                    f"⚠️ PDF सीधे नहीं भेज पाए, नीचे दिए गए बटन से download करें।",
                    reply_markup=markup,
                    parse_mode='Markdown'
                )

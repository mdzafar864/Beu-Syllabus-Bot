    ...
    def load(self):
        try:
            with open(self.filename, 'r') as f:
                ...
                logger.info(f"Analytics loaded successfully")
        except FileNotFoundError:
            logger.info("No existing analytics file, creating new one")
            self.save()
...
def broadcast_message(message):
    ...
    for user_id in analytics.total_users:
        try:
...
def user_stats(message):
    ...
    for cmd, count in sorted(analytics.command_stats.items(), key=lambda x: x[1], reverse=True):
...
try:
    ...
    def health_check():
        return {"status": "healthy", "users": len(analytics.total_users)}, 200

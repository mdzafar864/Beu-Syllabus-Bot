...
ADMIN_ID = int(os.getenv("ADMIN_ID", "5861904079"))  # Made configurable via env

# Optional: Webhook mode for better stability on Render
USE_WEBHOOK = os.getenv("USE_WEBHOOK", "false").lower() == "true"
WEBHOOK_URL = os.getenv("WEBHOOK_URL", "")  # Set in Render env: https://your-app.onrender.com/webhook
PORT = int(os.getenv("PORT", 8080))

# ================== SYLLABUS DATABASE (unchanged) ==================
SYLLABUS = {
    "1st New": {
...
# ================== FLASK WEB SERVER FOR RENDER (UPDATED) ==================
try:
    ...
    def run_web():
        ...
        if USE_WEBHOOK and WEBHOOK_URL:
            ...
            logger.info("⚠️ Webhook not configured, using polling mode")
    ...
    logger.warning(f"⚠️ Web server startup issue: {e}")

# ================== AUTO RESTART ON ERROR WITH BETTER HANDLING ==================
def run_bot():
    ...
    while True:
        try:
            ...
            if USE_WEBHOOK and WEBHOOK_URL:
                # In webhook mode, we don't need to poll - just keep the thread alive
                logger.info("📡 Bot running in webhook mode")
                while True:
                    time.sleep(60)  # Keep alive
            else:
                # Polling mode with better error handling

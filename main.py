...
# Store user states to avoid message conflicts
user_states = {}
...
def send_join_required(message):
    """Send join required message - always as new message"""
    try:
        # Delete previous join message if exists
        if message.chat.id in user_states:
            try:
                bot.delete_message(message.chat.id, user_states[message.chat.id])
            except:
                pass
        ...
        user_states[message.chat.id] = sent_msg.message_id
    except Exception as e:
...
def verify_join(call):
    try:
        ...
        if is_member(call.from_user.id):
            ...
            # Send alert that user hasn't joined
...
def branch_selected(message):
    try:
        ...
        # Create inline keyboard with Download and Share options
        ...
        bot.send_message(
            message.chat.id,
            "⚠️ Sorry, there was an error. Please try again.",
            reply_markup=MenuBuilder.main_menu()
        )
...
try:
    ...
    logger.warning("⚠️ Flask not installed")

...
def semester_after_branch(message):
    try:
        ...
        markup.add(
            InlineKeyboardButton("🔄 Select Another Branch", callback_data="select_another_branch"),
            InlineKeyboardButton("📖 Different Semester", callback_data=f"different_semester_{branch}")
        )
...
@bot.callback_query_handler(func=lambda call: call.data == "select_another_branch")
def select_another_branch(call):
    try:
        # Clear branch selection to start fresh
        user_session.set(call.from_user.id, "selected_branch", None)
        user_session.set(call.from_user.id, "step", "waiting_for_branch")
        
        # Show branch menu again
        ...
        bot.answer_callback_query(call.id, "✅ Select another branch")
        
    except Exception as e:
        logger.error(f"Error in select_another_branch: {e}")
        bot.answer_callback_query(call.id, "❌ Error")

@bot.callback_query_handler(func=lambda call: call.data.startswith("different_semester_"))
def different_semester(call):
    try:
        branch = call.data.replace("different_semester_", "")
        
        # Find available semesters for this branch
        available_semesters = []
        for sem, branches in SYLLABUS.items():
            if branch in branches:
                available_semesters.append(sem)
        
        user_session.set(call.from_user.id, "selected_branch", branch)
        user_session.set(call.from_user.id, "step", "waiting_for_semester")
        
        sem_text = f"📚 *{BRANCH_EMOJIS.get(branch, branch)} Branch*\n\n"
        sem_text += "अपना *Semester* चुनें:\n\n"
        for i, sem in enumerate(available_semesters, 1):
            sem_text += f"{i}. {sem}\n"
        ...
        bot.edit_message_text(
            ...
            call.message.chat.id,
            call.message.message_id,
            reply_markup=MenuBuilder.semester_for_branch_menu(branch, available_semesters),
            parse_mode='Markdown'
        )
        
        bot.answer_callback_query(call.id, "✅ Select semester")
        
    except Exception as e:
        logger.error(f"Error in different_semester: {e}")
        bot.answer_callback_query(call.id, "❌ Error")

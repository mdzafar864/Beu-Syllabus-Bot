...
# ================== HELPER FUNCTIONS ==================
def get_drive_links(drive_url):
    """Convert Google Drive download link to preview and view links"""
    try:
        ...
        if "id=" in drive_url:
            ...
            return drive_url, drive_url, drive_url
        ...
        return download_link, view_link, preview_link
...
def start(message):
    try:
        ...
        welcome_text = (
            ...
            "• Quick PDF downloads with preview\n\n"
            "💡 *How to use:*\n"
            "1️⃣ Click '📚 Syllabus'\n"
            "2️⃣ Select your semester\n"
            "3️⃣ Choose your branch\n"
            "4️⃣ View or download your syllabus!\n\n"
...
def show_semesters(message):
    try:
        ...
        if user_session.get(message.chat.id):
            user_session.set(message.chat.id, "semester", None)
        ...
        logger.error(f"Error in show_semesters: {e}")
...
def semester_selected(message):
    try:
        ...
        logger.error(f"Error in semester_selected: {e}")
...
# ================== MAIN SYLLABUS HANDLER WITH PREVIEW ==================
...
def branch_selected(message):
    try:
        ...
        if not branch or branch not in SYLLABUS[semester]:
            bot.send_message(message.chat.id, "❌ Invalid branch selection! Please try again.")
            return
        
        original_url = SYLLABUS[semester][branch]
        download_url, view_url, preview_url = get_drive_links(original_url)
        ...
        # Create inline keyboard with preview and download options
        markup = InlineKeyboardMarkup(row_width=2)
        markup.add(
            InlineKeyboardButton("👁️ View Online (Preview)", url=view_url),
            InlineKeyboardButton("⬇️ Download PDF", url=download_url)
        )
        markup.add(
            InlineKeyboardButton("📖 Open in Google Drive", url=preview_url),
        ...
        try:
            ...
            # Fallback: Send as message with buttons only
            bot.send_message(
                message.chat.id,
                f"📚 *{semester} Semester - {BRANCH_EMOJIS.get(branch, branch)} Syllabus*\n\n"
                f"✅ *Syllabus is ready!*\n\n"
                f"📊 *Downloads Today:* {analytics.daily_downloads.get(f'{semester}_{branch}', 0)}\n\n"
                f"👇 *Choose an option below to view or download:*",
                reply_markup=markup,
                parse_mode='Markdown'
            )
            
    except Exception as e:
        logger.error(f"Error in branch_selected: {e}")
        bot.send_message(
            message.chat.id,
...
def show_help(message):
    try:
        help_text = (
            ...
            "*Features:*\n"
            "• 👁️ View Online - Preview PDF in browser\n"
            "• ⬇️ Download PDF - Save to device\n"
            "• 📖 Open in Drive - View in Google Drive\n"
...
def broadcast_message(message):
    try:
        ...
        for user_id in list(analytics.total_users)[:100]:
            try:
